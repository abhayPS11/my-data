...
## Create custom Azure Linux 3.0 Arm image for the VM

In this learning path, we will start by downloading the AArch64 ISO of Azure Linux 3.0. Using qemu-img, we’ll create a raw disk image and boot a virtual machine with the ISO to install the OS onto the disk. Once the installation is complete, we will convert the raw disk to a fixed-size VHD, upload it to Azure Blob Storage, and then use the Azure CLI to create a custom ARM64 image. This custom image will enable us to launch Azure Linux 3.0 virtual machines on ARM-based infrastructure, even though the Azure Marketplace currently provides this image only for x64 systems. Next, switch to a Linux-based environment and proceed with the following steps.
...
This step boots the Azure Linux 3.0 ISO on an emulated ARM64 VM and installs the OS onto the raw disk.
...
After installing the Azure CLI, create a new resource group.
...
This deploys Azire Linux 3.0 Arm64 VM from the custom image. Confirm the same after visiting your Azure account “Virtual Machines” section.

...
cd gosort-bench
```
- **mkdir gosort-bench** - makes a new directory (folder) called gosort-bench.

- **cd gosort-bench** - goes inside that folder.
...
- **ARM64 maintains consistency** – both Docker containers and VMs delivered nearly identical results, showing that Arm64 optimizations are working well across environments.
...
- **No memory overhead** – both container and VM show **0 B/op and 0 allocs/op**, confirming Golang’s memory efficiency is preserved on Arm64.
